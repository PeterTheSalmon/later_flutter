...
import 'folder_icon_chooser.dart';

class FolderListTile extends StatefulWidget {
  const FolderListTile(
      {Key? key, required this.document, required this.showEditDelete})
      : super(key: key);
...
class _FolderListTileState extends State<FolderListTile> {
  ...
  Widget build(BuildContext context) {
    ...
    return ListTile(
      ...
      leading: getFolderIcon(widget.document["iconName"]),
      onTap: () {
        Navigator.push(
            context,
            displayMobileLayout
                ? fadeThrough(
                    (context, animation, secondaryAnimation) => FolderView(
                          ...
                          parentFolderName: widget.document["name"],
                        ))
                : FadeRoute(
                    page: FolderView(
                    parentFolderId: widget.document.id,
                    ...
                    final deleteSnackBar = SnackBar(
                      ...
                      action: SnackBarAction(
                        ...
                        onPressed: () {
                          ...
                          for (var element in _backupLinks) {
                            FirebaseFirestore.instance
                                .collection('links')
                                .doc(element.id)
                                .set({
